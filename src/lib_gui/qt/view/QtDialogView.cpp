	...
	std::function<void()> onCancelIndexing)
{
	...
	m_onQtThread(
		[=, this]()
		{
			...
			connect(
				window,
				&QtIndexingStartDialog::setShallowIndexing,
				[=, this](bool enabled) { m_shallowIndexingEnabled = enabled; });
			...
			connect(
				window,
				&QtIndexingStartDialog::startIndexing,
				[=, this](RefreshMode refreshMode)
				{
					...
					Task::dispatch(
						TabId::app(),
						std::make_shared<TaskLambda>([=, this]() { onStartIndexing(info); }));
			...
			connect(
				window,
				&QtIndexingDialog::canceled,
				[=, this]()
				{
					Task::dispatch(
						TabId::app(),
						std::make_shared<TaskLambda>([=, this]() { onCancelIndexing(); }));
	...
	bool shallow)
{
	...
	m_onQtThread(
		[=, this, &policy]()
		{
			...
			connect(
				window,
				&QtIndexingReportDialog::requestReindexing,
				[this, &policy]()
				{
					setUIBlocked(false);
